...
def test_large_button_affirmation_uses_only_neutral_audio() -> None:
    harness = make_harness()
    candidate = drive_to_private_voice_review(harness)
    ...
    assert harness.runtime.session.stage is SessionStage.PATIENT_CONFIRMED
    assert harness.runtime.session.selected_candidate_id == candidate.id
    assert harness.runtime.session.confirmation_method is (
    ...
    assert receipt is not None
    assert receipt.confirmation_method is ConfirmationMethod.LARGE_BUTTON
    assert receipt.voice_profile_id is None
    assert receipt.output_channel == "iphone_speaker"


def test_caregiver_cannot_use_large_button_for_neutral_confirmation() -> None:
    harness = make_harness()
...
def test_wrong_method_or_incomplete_readback_cannot_confirm() -> None:
    ...
    with pytest.raises(CommandRejected, match="semantic voice or large-button"):
        send(
            harness.runtime,
            PatientCommandType.CONFIRM_NEUTRAL_PLAYBACK,
            payload={
            ...
            confirmation_method=ConfirmationMethod.KEYBOARD,
        )
...
def test_l3_neutral_button_confirmation_requires_second_press() -> None:
    harness = make_harness(intent=L3IntentAdapter())
    drive_to_private_voice_review(harness)

    with pytest.raises(CommandRejected, match="another large-button"):
        send(
            harness.runtime,
            PatientCommandType.CONFIRM_NEUTRAL_PLAYBACK,
            payload={"private_readback_completed": True},
    ...
    send(
        ...
        payload={
            "private_readback_completed": True,
            "additional_button_confirmation": True,
        },
        confirmation_method=ConfirmationMethod.LARGE_BUTTON,
    )
